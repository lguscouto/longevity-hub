...
export const ProfileView: React.FC<ProfileViewProps> = ({ profile, onUpdateProfile }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Banner Card Header */}
      <div className="glass-panel p-6 rounded-3xl border border-emerald-500/30 bg-gradient-to-r from-slate-900 via-slate-900 to-emerald-950/40 relative overflow-hidden">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">{profile.email} • Perfil do Protocolo Blueprint</p>
            </div>
          </div>

          <button
            onClick={() => setIsEditing(!isEditing)}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 transition shadow-md"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800 bg-slate-900/60">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Idade Cronológica</span>
            <div className="p-2 rounded-xl bg-cyan-500/10 text-cyan-400">
              <Calendar className="h-5 w-5" />
            </div>
          </div>
          <div className="text-3xl font-extrabold text-white">
            {profile.chronological_age} <span className="text-xs text-slate-400 font-semibold">anos</span>
          </div>
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800 bg-slate-900/60">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Altura</span>
            <div className="p-2 rounded-xl bg-emerald-500/10 text-emerald-400">
              <Ruler className="h-5 w-5" />
            </div>
          </div>
          <div className="text-3xl font-extrabold text-emerald-400">
            {profile.height_cm} <span className="text-xs text-slate-400 font-semibold">cm ({heightInMeters} m)</span>
          </div>
          <span className="text-[10px] text-slate-500 mt-2 block">Sincronizado via Google Fit</span>
        </div>

        {/* Peso Atual */}
        <div className="glass-card p-5 rounded-2xl border border-slate-800 bg-slate-900/60">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Peso Atual</span>
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800 bg-slate-900/60">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Meta de Peso</span>
            <div className="p-2 rounded-xl bg-amber-500/10 text-amber-400">
              <Target className="h-5 w-5" />
            </div>
          </div>
          <div className="text-3xl font-extrabold text-amber-400">
            {profile.target_weight_kg} <span className="text-xs text-slate-400 font-semibold">kg</span>
          </div>
      ...
      {/* Edit Profile Form */}
      {isEditing && (
        <div className="glass-panel p-6 rounded-3xl border border-slate-800 bg-slate-900/90 shadow-2xl">
          <h3 className="text-base font-bold text-white mb-4 flex items-center gap-2">
            <Edit3 className="h-5 w-5 text-emerald-400" /> Editar Informações do Perfil
          </h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            ...
            <div>
              <label className="block text-slate-400 mb-1">Data de Nascimento</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-slate-400 mb-1">Meta de Peso (kg)</label>
              <input
                type="number"
                step="0.5"
            ...
            <div className="md:col-span-2 flex justify-end gap-3 mt-4">
              ...
              <button
                type="submit"
                className="px-5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold glow-emerald transition"
              >
                Salvar Alterações
              </button>
            </div>
