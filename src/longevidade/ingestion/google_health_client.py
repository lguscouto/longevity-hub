...
def format_rfc3339_utc(dt: datetime) -> str:
    """Formata datetime para string RFC 3339 UTC."""
...
def build_server_filter(
    ...
    - Datas formatadas em RFC 3339 UTC (ex: "2026-03-01T00:00:00Z")
...
def _build_raw_point(data_type: str, pt: Dict[str, Any], provider: str = "google_health") -> Dict[str, Any]:
    """Extrai e normaliza campos mínimos para persistência na camada raw health_data_points preservando raw_json."""
    import uuid
    pt_id = pt.get("id") or pt.get("dataPointId") or f"{provider}_{data_type}_{uuid.uuid4().hex[:16]}"
    source_val = pt.get("dataSource", {}).get("platform") or pt.get("source") or "GoogleHealthAPI"

    # Extração de tempos
    st_val = pt.get("startTime") or (pt.get("interval", {}).get("startTime", {}).get("physicalTime") if isinstance(pt.get("interval"), dict) else None)
    et_val = pt.get("endTime") or (pt.get("interval", {}).get("endTime", {}).get("physicalTime") if isinstance(pt.get("interval"), dict) else None)
...
class GoogleHealthClient:
    ...
    def fetch_data_points(
        ...
        """Busca dataPoints com paginação transparente e filtro temporal server-side."""
        ...
        # Filtro temporal client-side (buffer de segurança robusto)
    ...
    def reconcile(
        ...
        """Busca dataPoints reconciliados pela Google Health API (stream consolidado e deduplicado)."""
    ...
    def roll_up(
        ...
        """Busca agregados/roll-up de dados da Google Health API."""
        ...
        while True:
            ...
            data, err = self._get_json(url, call_params)
            if err:
                return all_aggregates, err
            if not data:
                break

            aggs = data.get("aggregates", []) or data.get("dataPoints", []) or data.get("dailyAggregates", [])
    ...
    def daily_roll_up(
        ...
        """Busca roll-up diário para métricas agregadas."""
        return self.roll_up(data_type=data_type, start_time=start_time, end_time=end_time, aggregation_period="DAILY")
